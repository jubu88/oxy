...
export interface EvalResult {
  score: number; // mean over the tasks
  rollouts: Rollout[];
}
...
export interface OptimizeDeps {
  ...
  epochs?: number;
  batchSize?: number;
  /** run builds with `skill` on `tasks`, score each, return mean + rollouts */
  evalSkill: (skill: string, tasks: Task[]) => Promise<EvalResult>;
...
export async function optimizeSkill(d: OptimizeDeps): Promise<OptResult> {
  ...
  const seed = await d.evalSkill(best, d.valTasks);
  ...
  for (let epoch = 0; epoch < epochs; epoch++) {
    for (let i = 0; i < d.trainTasks.length; i += batchSize) {
      ...
      // VALIDATION GATE: accept only if held-out score strictly improves
      const cand = await d.evalSkill(candidate, d.valTasks);
      const ok = cand.score > bestScore;
